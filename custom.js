(function (window, document) {
  ...
  function applyNavigationIcons() {
    ...
    Object.keys(iconMap).forEach(function (selector) {
      ...
      if (link.dataset.auroraIcon === iconMap[selector]) {
        return;
      }
      link.dataset.auroraIcon = iconMap[selector];
      Array.prototype.slice.call(link.querySelectorAll("i, span[class*='icon'], span.nav-bg-image, svg.ns-icon")).forEach(function (node) {
        node.remove();
      });
      link.insertBefore(iconElement(iconMap[selector]), link.firstChild);
      ...
      if (!link.getAttribute("title")) {
        link.setAttribute("title", link.getAttribute("aria-label") || iconMap[selector]);
      }
    });
  }
      ...
      var shouldUpdate = records.some(function (record) {
        return Array.prototype.some.call(record.addedNodes || [], function (node) {
          return node.nodeType === 1 && (
            ...
            (node.querySelector && node.querySelector("#menu-home, #menu-users, #menu-callcenter, #menu-conferences"))
          );
        });
      });
      if (shouldUpdate) {
